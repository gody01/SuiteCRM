...
class SuiteEditorSettingsForTinyMCE extends SuiteEditorSettingsForDirectHTML
{
    ...
    public $tinyMCESetupExtend = array(
             'tinyMCESetup' => "{
                 plugins: ['code','table','link','paste','searchreplace','link','image','wordcount',
                            'contextmenu','autolink','lists','charmap','print','anchor','textcolor',
                            'visualblocks','insertdatetime','preview','hr',  'colorpicker'],
                 toolbar: ['undo redo | bold italic backcolor | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | removeformat | link' ,
                           'styleselect fontselect fontsizeselect | code'],
             }",
    );   

    public function __construct($settings = null) {
        parent::__construct($settings);
        if($this->tinyMCESetupExtend) {
            $this->extend($this->tinyMCESetupExtend);
        }
    }
